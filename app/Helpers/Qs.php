...
class Qs
{
    ...
    public static function userIsPlatformSuperAdmin()
    {
        return auth()->check() && auth()->user()->isPlatformSuperAdmin();
    }

    public static function userIsSchoolAdmin($branchId = null)
    {
        return auth()->check() && auth()->user()->isSchoolAdmin($branchId);
    }

    public static function userIsStaff($branchId = null)
    {
        if ($branchId !== null) {
            return auth()->check() && auth()->user()->isStaff($branchId);
        }
        return auth()->check() && in_array(auth()->user()->user_type, self::getStaff());
    }

    public static function userCanManageBranches()
    {
        return auth()->check() && auth()->user()->hasPermission('manage_branches');
    }

    public static function userCanAccessBranch($branchId)
    {
        return auth()->check() && auth()->user()->canAccessBranch($branchId);
    }

    public static function getUserAccessibleBranches()
    {
        return auth()->check() ? auth()->user()->getAccessibleBranches() : collect();
    }
